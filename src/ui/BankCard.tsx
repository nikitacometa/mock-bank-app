...
const FACES: Record<Card['design'], { bg: string; ink: string; dim: string; trail: string }> = {
  ...
  ivory: {
    ...
    // Holder and expiry are small text: keep at least 4.5:1 over the terminal stop.
    dim: 'oklch(0.23 0.015 260 / 78%)',
    ...
    >
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <div className="mt-1 text-[0.6875rem] tracking-[0.08em]" style={{ color: face.dim }}>
              {localizeDemoText(card.holder, locale)}
            </div>
          </div>
      ...
      {frozen && (
        <div className="absolute inset-0 flex items-center justify-center gap-2 backdrop-blur-[2px]">
          <IconFreeze size={18} />
          <span className="text-[0.9375rem] font-medium">{t('cards.frozen')}</span>
        </div>
      )}
    </div>
  );
